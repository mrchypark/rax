...
const DOC_SEGMENT_MAGIC: &[u8; 4] = b"RXDG";
...
impl DocIdBinding {
    pub fn new(rax_doc_id: u64, external_doc_id: impl Into<String>) -> Self {
        Self {
            rax_doc_id,
...
pub struct DocIdMap {
    ...
    by_rax_doc_id: HashMap<u64, String>,
}
...
impl DocIdMap {
    ...
    pub fn from_bindings(bindings: Vec<DocIdBinding>) -> Result<Self, DocstoreError> {
        ...
        let mut by_rax_doc_id = HashMap::with_capacity(bindings.len());

        for binding in &bindings {
            if by_external_doc_id
                .insert(binding.external_doc_id.clone(), binding.rax_doc_id)
                .is_some()
            {
            ...
            if by_rax_doc_id
                .insert(binding.rax_doc_id, binding.external_doc_id.clone())
                .is_some()
            {
                return Err(DocstoreError::InvalidDocument(
                    "duplicate rax_doc_id binding".to_owned(),
                ));
            }
        }
    ...
    pub fn rax_doc_id(&self, external_doc_id: &str) -> Option<u64> {
        self.by_external_doc_id.get(external_doc_id).copied()
    }

    pub fn external_doc_id(&self, rax_doc_id: u64) -> Option<&str> {
        self.by_rax_doc_id.get(&rax_doc_id).map(String::as_str)
    }
    ...
    pub fn next_rax_doc_id(&self) -> Result<u64, DocstoreError> {
        Ok(self
            .bindings
            .iter()
            .map(|binding| binding.rax_doc_id)
            .max()
            .map(|value| {
                value.checked_add(1).ok_or_else(|| {
                    DocstoreError::InvalidDocument("rax_doc_id space exhausted".to_owned())
                })
            })
            .transpose()?
            .unwrap_or(0))
    ...
    pub fn extend_to_cover_document_order(
        ...
        let mut next_rax_doc_id = self.next_rax_doc_id()?;
        ...
        for document_id in document_ids {
            ...
            if self.rax_doc_id(document_id).is_none() {
                bindings.push(DocIdBinding::new(next_rax_doc_id, document_id.clone()));
                next_rax_doc_id = next_rax_doc_id.checked_add(1).ok_or_else(|| {
                    DocstoreError::InvalidDocument("rax_doc_id space exhausted".to_owned())
                })?;
            }
        }

        bindings.sort_by_key(|binding| binding.rax_doc_id);
        Self::from_bindings(bindings)
    }
    ...
    pub fn bindings_for_external_doc_ids_sorted(
        ...
        for document_id in document_ids {
            ...
            let rax_doc_id = self.rax_doc_id(document_id).ok_or_else(|| {
                DocstoreError::InvalidDocument(format!(
                    "missing rax doc id binding for {document_id}"
                ))
            })?;
            bindings.push(DocIdBinding::new(rax_doc_id, document_id.clone()));
        }
...
struct StoreDocSegment {
    minor: u16,
    bytes: Arc<rax_core::SegmentObject>,
...
impl StoreDocSegment {
        ...
        for row_index in 0..row_count {
            ...
            if let Some(doc_id_map) = doc_id_map.as_ref() {
                if doc_id_map.external_doc_id(row.doc_id).is_none() {
                    return Err(DocstoreError::InvalidDocument(format!(
                        "missing external doc_id binding for rax_doc_id {}",
                        row.doc_id
                    )));
                }
            }
    ...
    fn load_documents_by_id(
        ...
        for doc_id in target_doc_ids {
            let Some(rax_doc_id) = doc_id_map.rax_doc_id(doc_id) else {
                continue;
            };
            let Some(row) = self.find_row_by_rax_doc_id(rax_doc_id) else {
                continue;
            };
    ...
    fn find_row_by_rax_doc_id(&self, rax_doc_id: u64) -> Option<DocRow> {
        let mut low = 0usize;
        let mut high = self.row_count;
        while low < high {
            let mid = low + (high - low) / 2;
            let row = self.row_at(mid);
            match row.doc_id.cmp(&rax_doc_id) {
...
impl Docstore {
    ...
    pub fn publish_to_store(&self, store_path: &Path) -> Result<OpenedStore, DocstoreError> {
        ...
        rax_core::publish_segments_with_precondition(store_path, vec![prepared], |manifest| {
...
fn build_binary_doc_segment_from_documents(
    ...
    for binding in active_bindings {
        ...
        let rax_doc_id = binding.rax_doc_id;
...
fn open_read_no_symlinks(path: &Path) -> Result<File, DocstoreError> {
    rax_core::open_file_read_no_symlinks(path)
        .map_err(|error| DocstoreError::InvalidDocument(error.to_string()))
}
...
mod tests {
    ...
    fn doc_id_map_assigns_numeric_ids_in_document_file_order() {
        ...
        assert_eq!(doc_id_map.rax_doc_id("doc-010"), Some(1));
    ...
    fn doc_id_map_changes_when_document_file_order_changes() {
        ...
        assert_eq!(first.rax_doc_id("doc-a"), Some(0));
        assert_eq!(second.rax_doc_id("doc-a"), Some(1));
    }
    ...
    #[test]
    fn doc_id_map_rejects_exhausted_rax_doc_id_space() {
    ...
    #[test]
    fn open_store_segment_uses_binding_section_for_doc_ids_in_current_format() {
        let temp_dir = tempdir().unwrap();
        let docs_path = temp_dir.path().join("docs.ndjson");
        let offsets_path = temp_dir.path().join("document-offsets.jsonl");
        let store_path = temp_dir.path().join("store.rax");
    ...
    fn test_doc_id_map(bindings: &[(u64, &str)]) -> DocIdMap {
        DocIdMap::from_bindings(
            bindings
                .iter()
                .map(|(rax_doc_id, external_doc_id)| {
                    DocIdBinding::new(*rax_doc_id, (*external_doc_id).to_owned())
                })
                .collect(),
        )
        .unwrap()
    ...
    fn encode_minor_v0_segment(segment: &BinaryDocSegment) -> Result<Vec<u8>, DocstoreError> {
        let minor_current = segment.encode()?;
        let payload_bytes_offset = read_u64(&minor_current, 16) as usize;
        let metadata_bytes_offset = read_u64(&minor_current, 24) as usize;
        let preview_bytes_offset = read_u64(&minor_current, 32) as usize;
        let binding_bytes_offset = read_u64(&minor_current, 40) as usize;
        let row_table_offset = read_u64(&minor_current, 48) as usize;
        let row_count = read_u64(&minor_current, 8);

        let payload_section = &minor_current[payload_bytes_offset..metadata_bytes_offset];
        let metadata_section = &minor_current[metadata_bytes_offset..preview_bytes_offset];
        let preview_section = &minor_current[preview_bytes_offset..binding_bytes_offset];
        let row_table = &minor_current[row_table_offset..];
