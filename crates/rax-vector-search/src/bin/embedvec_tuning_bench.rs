...
use rax_vector_search::embedvec_backend::{EmbedVecConfig, EmbedVecVectorEngine};
...
fn main() {
    ...
    for variant in variants {
        ...
        let result = run_embedvec_bench(variant, &corpus, &queries, &truth, top_k);
        eprintln!(
...
fn run_embedvec_bench(
    variant: EmbedVecVariant,
    corpus: &[Vec<f32>],
    queries: &[Vec<f32>],
    truth: &[Vec<u64>],
    top_k: usize,
) -> BenchResult {
    let mut engine = EmbedVecVectorEngine::with_config(variant.config);
