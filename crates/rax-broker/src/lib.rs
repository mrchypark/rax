...
pub struct RaxBroker {
    next_session_id: u64,
    max_sessions: usize,
    sessions: HashMap<SessionId, RuntimeStore>,
}

impl Default for RaxBroker {
    fn default() -> Self {
        Self::new()
    }
}
...
fn runtime_error(error: rax_runtime::RuntimeError) -> BrokerError {
    match error {
        rax_runtime::RuntimeError::InvalidRequest(message) => BrokerError::InvalidRequest(message),
        rax_runtime::RuntimeError::Storage(message) => BrokerError::Storage(message),
    }
}
...
mod tests {
    ...
    use crate::{BrokerError, RaxBroker};

    #[test]
    fn broker_rejects_new_sessions_after_configured_capacity() {
        let mut broker = RaxBroker::with_max_sessions(0);
