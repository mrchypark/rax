...
impl RuntimeStore {
    ...
    pub fn create(root: &Path) -> Result<Self, RuntimeError> {
        ...
        let created_file = wax_v2_core::create_empty_store_and_open(&store_path)
    ...
    pub fn create_at(path: &Path) -> Result<Self, RuntimeError> {
        ...
        let created_file = match wax_v2_core::create_empty_store_and_open(&store_path) {
            Ok(file) => file,
...
impl RuntimeStoreWriter<'_> {
    ...
    fn publish_raw_snapshot_with_expected_generation(
        ...
        if let Some(vectors) = vectors {
            ...
            let active_document_ids =
                wax_v2_docstore::order_document_ids_for_store(&store_path, &document_ids)
                    .map_err(|error| RuntimeError::Storage(docstore_error(error)))?;
    ...
    pub fn publish_staged_compatibility_snapshot(
        ...
        let vectors =
            wax_v2_vector::load_compatibility_raw_vectors(&root_path, &self.store.manifest)
                .map_err(RuntimeError::Storage)?
                .into_iter()
...
fn load_memory_vectors(
    ...
    let raw_vectors = wax_v2_vector::load_runtime_raw_vectors_with_store_path(
        &store.root_path(),
        &store.manifest,
        &store.store_path(),
    )
...
fn vector_inputs_sorted_by_wax_doc_id(
    ...
    let doc_id_end_exclusive = vector_inputs
        ...
        .map(|(wax_doc_id, _, _)| {
            wax_doc_id
                .checked_add(1)
                .ok_or_else(|| RuntimeError::Storage("wax_doc_id space exhausted".to_owned()))
        })
        .transpose()?
...
fn cleanup_created_empty_store_file(path: &Path, expected_identity: Option<&FileIdentity>) {
    ...
    let Ok(opened) = wax_v2_core::open_store(path) else {
        return;
    };
...
fn load_compatibility_raw_documents(
    ...
    BufReader::new(
        wax_v2_core::open_file_read_no_symlinks(&documents_path).map_err(runtime_core_error)?,
    )
    .lines()
...
fn read_manifest(root: &Path) -> Result<DatasetPackManifest, RuntimeError> {
    let manifest_bytes = wax_v2_core::read_file_no_symlinks(&root.join("manifest.json"))
        .map_err(runtime_core_error)?;
...
fn validate_prebuilt_store_segments_against_dataset_pack(
    ...
    wax_v2_docstore::validate_store_segment_against_dataset_pack_with_store_path(
        store_path, root, manifest,
    )
    .map_err(|error| RuntimeError::Storage(docstore_error(error)))?;
    wax_v2_text::validate_store_segment_against_dataset_pack_with_store_path(
        store_path, root, manifest,
    )
    .map_err(RuntimeError::Storage)?;
    wax_v2_vector::validate_store_segment_against_dataset_pack_with_store_path(
        store_path, root, manifest,
    )
    .map_err(RuntimeError::Storage)?;
    Ok(())
...
fn stable_existing_store_path(
    ...
    let file = match mode {
        ProductStoreOpenMode::ReadOnly => {
            wax_v2_core::open_file_read_no_symlinks(path).map_err(runtime_core_error)?
        }
        ProductStoreOpenMode::ReadWrite => {
            wax_v2_core::open_file_readwrite_no_symlinks(path).map_err(runtime_core_error)?
        }
    };
    #[cfg(target_os = "linux")]
    {
...
fn stable_linux_proc_fd_child_store_path(
    ...
    let file = match mode {
        ProductStoreOpenMode::ReadOnly => {
            wax_v2_core::open_file_read_no_symlinks(&stable_path).map_err(runtime_core_error)?
        }
        ProductStoreOpenMode::ReadWrite => {
            wax_v2_core::open_file_readwrite_no_symlinks(&stable_path)
                .map_err(runtime_core_error)?
        }
    };
    let identity = runtime_file_identity_from_file(&file)?;
...
fn store_path_from_manifest(
    ...
    manifest_file_by_kind(manifest, "store")
        ...
        .unwrap_or_else(|| Ok(root.join("store.wax")))
}
...
mod tests {
    ...
    fn runtime_store_identity_failure_fails_open_and_create_closed() {
        let temp_dir = tempdir().unwrap();
        let store_path = temp_dir.path().join("agent.wax");
        let new_store_path = temp_dir.path().join("new-agent.wax");
    ...
    fn memory_open_creates_missing_parent_directories() {
        let temp_dir = tempdir().unwrap();
        let store_path = temp_dir.path().join("nested").join("agent.wax");
    ...
    fn memory_open_does_not_create_parent_dirs_through_symlink_component() {
        ...
        let store_path = link.join("nested").join("agent.wax");
    ...
    fn memory_open_keeps_created_store_bound_after_parent_path_swap() {
        ...
        let store_path = parent.join("agent.wax");
        ...
        crate::set_first_create_post_create_hook(move || {
            ...
            create_empty_store(&hook_parent.join("agent.wax")).unwrap();
        });
        ...
        let mut original = Memory::open_existing(&moved_parent.join("agent.wax")).unwrap();
        ...
        let mut replacement = Memory::open_existing(&parent.join("agent.wax")).unwrap();
    ...
    fn memory_open_existing_keeps_store_bound_after_parent_path_swap() {
        ...
        fs::create_dir(&parent).unwrap();
        create_empty_store(&parent.join("agent.wax")).unwrap();
    ...
    fn runtime_store_create_at_rejects_existing_invalid_store_file() {
        let temp_dir = tempdir().unwrap();
        let store_path = temp_dir.path().join("agent.wax");
        fs::write(&store_path, b"not a wax store").unwrap();
    ...
    fn product_store_root_defaults_simple_relative_paths_to_current_directory() {
        ...
        assert_eq!(
            super::product_store_path_under_root(Path::new("."), Path::new("agent.wax")).unwrap(),
            PathBuf::from("agent.wax")
        );
        assert_eq!(
            super::product_store_path_under_root(Path::new("."), Path::new("./agent.wax")).unwrap(),
            PathBuf::from("agent.wax")
    ...
    fn runtime_store_open_existing_preserves_proc_self_fd_store_path() {
        ...
        let fd_store_path =
            PathBuf::from(format!("/proc/self/fd/{}/agent.wax", root_dir.as_raw_fd()));
        ...
        drop(root_dir);
        let opened = wax_v2_core::open_store(&stable_store_path).unwrap();
    ...
    fn runtime_store_create_rejects_manifest_store_path_outside_root() {
        let dataset_dir = tempdir().unwrap();
        let outside_store = dataset_dir.path().join("..").join("outside.wax");
        let mut manifest = product_manifest(
            dataset_dir.path(),
            &dataset_dir.path().join("placeholder.wax"),
        )
        .unwrap();
        manifest.files[0].path = "../outside.wax".to_owned();
        fs::write(
            dataset_dir.path().join("manifest.json"),
            serde_json::to_string(&manifest).unwrap(),
        )
