...
use rax_bench_model::{
    ...
    MountRequest, OpenRequest, OpenResult, RankedDocumentHit, RankedQueryResult, RaxEngine,
    SearchRequest, SearchResult, VectorQueryMode,
};
...
use rax_text::{TextBatchQuery, TextLane};
use rax_vector::{elapsed_ms, VectorLane};
use serde::Serialize;
...
impl PackedTextEngine {
    ...
    fn core_store_path(&self) -> Result<PathBuf, String> {
        Ok(self.mount_root()?.join("store.rax"))
    }
...
impl RaxEngine for PackedTextEngine {
    type Error = String;
    ...
    fn open(&mut self, _request: OpenRequest) -> Result<OpenResult, Self::Error> {
        ...
        if core_store_path.exists() {
            rax_core::open_store(&core_store_path)
...
mod tests {
    ...
    use rax_bench_model::VectorQueryMode;
    use rax_docstore::parse_document_id;
    use rax_search::{filter_hits_by_metadata, MetadataFilter};
    use rax_vector::resolve_auto_vector_mode;
